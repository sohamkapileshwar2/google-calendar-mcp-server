...
class CalendarListEntryInsertQueryParams(BaseModel):
    """
    Query Parameter Schema for inserting a CalendarList entry (POST /users/me/calendarList/{calendarId})
...
class CalendarListEntryInsert(BaseModel):
    """
    Schema for inserting a CalendarList entry (POST /users/me/calendarList/{calendarId})
    Fields are all optional—only, except id which is required. Include what you're changing.
    """
    id: str = Field(
        ...,
